...
impl Window {
    ...
    /// Renders the window border and layout details.
    pub fn draw<W: Write>(&self, w: &mut W, rect: Rect) -> std::io::Result<()> {
        ...
        if rect.width > 2 {
            ...
            if title_len + 4 < rect.width as usize {
                ...
                execute!(w, Print(format!("┌{}┐", "─".repeat(rect.width as usize - 2))))?;
            }
        } else {
            execute!(w, Print("┌┐".chars().take(rect.width as usize).collect::<String>()))?;
        }
